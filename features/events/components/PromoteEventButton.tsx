...
/**
 * Promote Event Button - Same system as products
 */
...
export function PromoteEventButton({ eventId, eventTitle, isPromoted, promotedUntil }: Props) {
  ...
  const handleSelect = (packageId: number) => {
    ...
    startTransition(async () => {
      const result = await createPromotionPurchase(eventId, packageId, 'event')
      if (result.success) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setIsOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b">
              <h2 className="text-xl font-bold">Promouvoir votre événement</h2>
