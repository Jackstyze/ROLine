...
/**
 * Promote Button - Opens promotion modal for product
 */

import { useState, useTransition } from 'react'
import { getPromotionPackages, createPromotionPurchase, type PromotionPackage } from '../actions/promotion.actions'
...
export function PromoteButton({ productId, productTitle, isPromoted, promotedUntil }: PromoteButtonProps) {
  ...
  const handleSelect = (packageId: number) => {
    ...
    startTransition(async () => {
      const result = await createPromotionPurchase(productId, packageId)
      if (result.success) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setIsOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b">
              <h2 className="text-xl font-bold">Promouvoir votre produit</h2>
              <p className="text-gray-600 text-sm mt-1">{productTitle}</p>
            </div>
